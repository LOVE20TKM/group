...
echo "========================================="
echo "Prechecking GroupDelegate Deployment"
echo "========================================="
...
address_file="$network_dir/address.group.delegate.params"
if [ -f "$address_file" ]; then
    source "$address_file"

    if [ -n "$groupDelegateAddress" ]; then
        delegate_code=$(cast code "$groupDelegateAddress" --rpc-url "$RPC_URL" 2>/dev/null)
        if [ -n "$delegate_code" ] && [ "$delegate_code" != "0x" ]; then
            bound_group_address=$(cast call "$groupDelegateAddress" "GROUP_ADDRESS()(address)" --rpc-url "$RPC_URL" 2>/dev/null)
            echo -e "\033[33mWarning:\033[0m Existing GroupDelegate deployment detected"
            echo "  GroupDelegate Address: $groupDelegateAddress"
            ...
            if [ "$(printf '%s' "$bound_group_address" | tr '[:upper:]' '[:lower:]')" = "$(printf '%s' "$groupAddress" | tr '[:upper:]' '[:lower:]')" ]; then
                if [ "$FORCE_REDEPLOY" != "1" ]; then
                    echo -e "\033[31mError:\033[0m Refusing to redeploy while address.group.delegate.params already points to live code for the current group"
                ...
                echo -e "\033[33mWarning:\033[0m Existing GroupDelegate is bound to a different group, continuing with redeploy"
            fi
        else
            echo -e "\033[33mWarning:\033[0m Existing address.group.delegate.params found, but no live code at that address"
        fi
    fi
fi

export GROUP_DELEGATE_PRECHECK_DONE=1
export GROUP_DELEGATE_PRECHECK_KEY="$current_precheck_key"
echo -e "\033[32m✓\033[0m GroupDelegate deployment precheck passed"
echo "========================================="
